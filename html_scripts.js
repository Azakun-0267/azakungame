...
function fillCpuLevelSelects(){ [1,2,3].forEach(n=>{ const sel=$("cpu"+n+"LevelSelect"); if(sel && !sel.__filled){ for(let i=0;i<=9;i++){ const o=document.createElement("option"); o.value=String(i); o.textContent=i===0?"OFF":String(i); if((n===1&&i===5)||(n>1&&i===0)) o.selected=true; sel.appendChild(o);} sel.__filled=true; } }); }
fillCpuLevelSelects();
...
Object.defineProperty(window, "currentRoom", {get:()=>currentRoom, set:v=>{currentRoom=v;}});
let mySlot = null;
let roomState = null;
...
function updatePlayerList(){
  ...
  for(const p of roomState.players){
    ...
    const teamName = ["", "赤", "青", "緑", "黄"][p.team || 1] || "赤";
    const canTeam = roomState.status === "lobby" && (roomState.hostId === socket.id || p.id === socket.id);
    const team = `<br>チーム：${teamName}${canTeam ? `<br><select class="teamSelect" data-pid="${p.id}"><option value="1" ${p.team===1?"selected":""}>赤</option><option value="2" ${p.team===2?"selected":""}>青</option><option value="3" ${p.team===3?"selected":""}>緑</option><option value="4" ${p.team===4?"selected":""}>黄</option></select>` : ""}`;

    const box = document.createElement("div");
    box.className = "playerBox";
    box.innerHTML = `<b>${p.name}${host}</b><br>キャラ：${charName}<br>ステージ候補：${stageName}${team}`;
  ...
  if(cpuCountSel) {
    cpuCountSel.disabled = !isHost;
    cpuCountSel.value = String(roomState.cpuCount ?? cpuCountSel.value ?? 1);
  }
  const lvArr = roomState.cpuLevels || [roomState.cpuLevel || 5,0,0];
  [1,2,3].forEach(n => {
    const sel = $("cpu"+n+"LevelSelect");
    if(sel){ sel.disabled = !isHost; sel.value = String(lvArr[n-1] ?? (n===1?5:0)); }
  });
  const teamCheck = $("teamModeCheck");
  if(teamCheck){
    teamCheck.checked = !!roomState.teamMode;
    teamCheck.disabled = roomState.hostId !== socket.id;
  }
  document.querySelectorAll(".teamSelect").forEach(sel => {
    sel.onchange = () => socket.emit("setPlayerTeam", {code:currentRoom, playerId:sel.dataset.pid, team:Number(sel.value)});
  });
...
$("createBtn").onclick = () => {
  ...
  socket.emit("createRoom", {
    ...
    cpuLevel:Number(document.getElementById("cpu1LevelSelect")?.value ?? 5),
    cpuLevels:[1,2,3].map(n=>Number(document.getElementById("cpu"+n+"LevelSelect")?.value ?? (n===1?5:0))),
    cpuCount:Number(document.getElementById("cpuCountSelect")?.value ?? 1),
    teamMode:!!document.getElementById("teamModeCheck")?.checked
  });
};
...
$("spectateBtn").onclick = () => {
  ...
  socket.emit("spectateRoom", {
    ...
    guestName:$("usernameInput").value || "Spectator"
  });
};
...
function cpuLevels(){
  const count = Number($("cpuCountSelect")?.value ?? 1);
  return [1,2,3].map(n=>{
    const sel = $("cpu"+n+"LevelSelect");
    let v = Number(sel?.value ?? (n===1?5:0));
    if(n <= count && v <= 0){ v = 5; if(sel) sel.value = "5"; }
    return v;
  });
}
function emitCPUSetting(){
  initAudio();
  const levels = cpuLevels();
...
["cpu1LevelSelect","cpu2LevelSelect","cpu3LevelSelect","cpuCountSelect"].forEach(id=>{ const el=$(id); if(el) el.onchange = emitCPUSetting; });
function emitTeamSetting(){
  if(currentRoom) socket.emit("setTeamMode", {code:currentRoom, teamMode:!!$("teamModeCheck")?.checked});
}
if($("teamModeCheck")) $("teamModeCheck").onchange = emitTeamSetting;
...
socket.on("joinedRoom", data => {
  ...
  if(roomState){ const a=roomState.cpuLevels || [roomState.cpuLevel||5,0,0]; [1,2,3].forEach(n=>{ if($("cpu"+n+"LevelSelect")) $("cpu"+n+"LevelSelect").value=String(a[n-1]??0); }); }
  renderAllCards();
...
function drawFighter(p){
  try{
    ...
    if(roomState && roomState.teamMode){
      const tc = [null,"#ff3b3b","#3b8cff","#39d353","#ffd33d"][p.team || 1] || "#ffffff";
      ctx.save();
      ctx.strokeStyle = tc;
      ctx.lineWidth = 5;
      ctx.globalAlpha = 0.95;
      ctx.strokeRect(x-5,y-7,p.w+10,p.h+14);
      ctx.restore();
    }
    ...
    if(p.ledgeHang){
      ctx.fillStyle = "#ffffff";
      ctx.font = "12px Arial";
      ctx.fillText("崖 " + Math.ceil((p.ledgeTimer||0)/60), x-2, y-12);
    }
...
(function(){
  ...
  function boot(){
    [
      ...
      "readyBtn","leaveRoomBtn","backTitleBtn","backLobbyBtn","spectateBtn"
    ].forEach(enable);
  }
...
(function(){
  ...
  function cpuLevel(){
    const sel = $("cpu1LevelSelect");
    return Math.max(0, Math.min(9, Number(sel ? sel.value : 5)));
  }
  function cpuLevels(){
    return [1,2,3].map(function(n){ var sel=$("cpu"+n+"LevelSelect"); return Math.max(0, Math.min(9, Number(sel ? sel.value : (n===1?5:0)))); });
  }
  function cpuCount(){
  ...
  function bindCPUFix(){
    ...
    if(sel && !sel.__cpuFixBound){
      ...
      sel.addEventListener("change", function(){
        try{
          if(window.socket && window.currentRoom) socket.emit("setCPU", {code:currentRoom, level:cpuLevel(), levels:cpuLevels(), count:cpuCount()});
      ...
      start.addEventListener("click", function(){
        try{
          if(window.socket && window.currentRoom){
            socket.emit("setCPU", {code:currentRoom, level:cpuLevel(), levels:cpuLevels(), count:cpuCount()});
    ...
    if(create && !create.__cpuCreateFixBound){
      ...
      create.addEventListener("click", function(){
        try{
          window.__pendingCpuLevel = cpuLevel(); window.__pendingCpuLevels = cpuLevels(); window.__pendingCpuCount = cpuCount();
        }catch(e){}
      }, true);
    }
  }
